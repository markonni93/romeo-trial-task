...
const getProfilesEndpoint = "/api/search?";
const getProfileDetailsEndpoint = "/api/profiles?";

const getProfiles = async (cursor?: string): Promise<ProfileModel[]> => {
  try {
    const searchParams = new URLSearchParams();
    searchParams.append("length", "32");
    searchParams.append("cursor", cursor ?? "");

    const profilesRequest = await fetch(
      `${baseUrl}${getProfilesEndpoint}${searchParams.toString()}`
    );
    ...
    const profileDetailsRequest = await fetch(
      `${baseUrl}${getProfileDetailsEndpoint}${params.toString()}`
    );
    const profileDetailResponse =
      (await profileDetailsRequest.json()) as RomeoProfileDetailResponse[];
